...
import 'package:arti_stone_crusher/data/repo/invoice_repo.dart';
...
class CreateInvoice extends StatefulWidget {
  ...
  const CreateInvoice({Key? key, this.isEdit = false, this.data, this.callback})
      : super(key: key);
...
class _CreateInvoiceState extends State<CreateInvoice> {
  final InvoiceRepo repo = InvoiceRepo();
  ...
  Future fetchMeasurementId() async {
    try {
      Map<String, dynamic> data = {
        "party_id": selectParty,
        "vehicle_id": selectVehicle==null?null:jsonDecode(selectVehicle!)['vehicle_id']
      };
      // emit(state.copyWith(loginApiStatus: ApiStatus.loading));
  ...
  Future fetchPrices() async {
    try {
      ...
      // emit(state.copyWith(loginApiStatus: ApiStatus.loading));
      var res = await repo.fetchPrice(data);
      print('api resoonse is price ${res}');
      // if (res['result']['success'].toString() == "1") {

      priceController.text = res['rate'];
      // } else {
      //   throw res['result']['error_msg'];
      // }
    } catch (e, s) {
  ...
  Future<void> fetchEditList() async {
    try {
      ...
      final dio = Dio();
      final response =
      await dio.post(NetworkConfig.baseUrl + NetworkConfig.selectInvoice,
      ...
      selectParty == null ||
        ...
        if ( selectPaymentMethod != 'Cash') {
          if (upiIdController.text.isEmpty) {
            Fluttertoast.showToast(msg: 'Please enter your Transaction ID');
            return;
          }

          // if (!upiIdController.text.contains('@') || !upiIdController.text.contains('.')) {
    ...
    try {
      Map<String, dynamic> data = {
        ...
        "price":(double.tryParse(priceController.text) ?? 0) *
            (double.tryParse(measurementController.text) ?? 0),
      ...
      // emit(state.copyWith(loginApiStatus: ApiStatus.loading));
      var res = await repo.createInvoice(data);
      print('api resoonse is ${res}');
      if (res['result']['success'].toString() == "1") {
        ...
        if(isPdf) {
          ...
          final tripId = res['result'][
            //'invoce_id'
            'invoice_id'
          ];
          final Uri url = Uri.parse(
            // res['result']['url']
              'https://www.artistonecrusher.com/index/print_bill/$tripId'
          );
        ...
        Navigator.pushReplacement(
            context, MaterialPageRoute(builder: (context) => InvoiceList()));
  ...
  void   fetchUpdateChallan() async {
    ...
    try {
      ...
      isLoadingButton.value = true;
      // emit(state.copyWith(loginApiStatus: ApiStatus.loading));
      var res = await repo.updateInvoice(data);
      print('api resoonse is ${res}');
      if (res['result']['success'].toString() == "1") {
        print('check list is ${res['result']['data']}');
        Fluttertoast.showToast(msg: res['result']['msg']);
  ...
  void editFieldUpdate(Map<String, dynamic> data1) {
    ...
    priceController.text = data1['rate'];
    // creditType = data1['credit'].toString() == '1'
    //     ? "Credit"
    //     : data1['credit'].toString() == '2'
    //     ? 'Non Credit'
    //     : ''; // or default
    selectPaymentMethod = data1['upi_id'].toString() ==''?"Cash":"UPI";
    upiIdController.text =data1['upi_id'].toString() ==''?"":data1['upi_id'].toString();
    // data1['cash_upi'].toString() == '1'
    //     ? "Cash"
  ...
  totalPrice(){
    if(priceController.text.isNotEmpty &&  measurementController.text.isNotEmpty){
      totalAmountController.text = ((double.tryParse(priceController.text) ?? 0) *
          (double.tryParse(measurementController.text) ?? 0)).toString();
    }
                            ...
                            Expanded(
                                child: ValueListenableBuilder(
                                  ...
                                  builder: (context, bool loading, child) =>
                                      AppButton(
                                        fontSize: 14,
                                        backgroundColor: totalAmountController.text.isEmpty?Colors.grey:Color(0xffE8710F),
                                        text: widget.isEdit ? 'Update and Go to List' : 'Save and Go to List',
                                        onPressed:totalAmountController.text.isEmpty?null: () {
                                          if (widget.isEdit) {
                                            fetchUpdateChallan();
                                          } else {
                                            fetchSubmitChallan();
            ...
            topWidget: Padding(
              ...
              child: Row(
                ...
                children: [
                  ...
                  Text(
                    '${widget.isEdit ? "Update" : "Create"} Invoice',
                    style: TextStyle(
                        fontWeight: FontWeight.w600,
                        color: Colors.white,
                        fontSize: 20),
